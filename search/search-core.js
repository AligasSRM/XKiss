(() => {
  ...
  function getSearchScore(video, query) {
    ...
    const tags = (Array.isArray(video.tags) ? video.tags : []).map(normalize).filter(Boolean);
    let score = 0;
  ...
  function matches(video) {
    ...
    if (selectedCategory && selectedCategory !== "all") {
      ...
      if (category !== selectedCategory && !tags.includes(selectedCategory)) return false;
    }
    if (!query) return true;
    const searchable = [video.title, video.creator, video.category, ...(Array.isArray(video.tags) ? video.tags : [])].filter(Boolean).join(" ").toLowerCase();
  ...
  function apply() {
    ...
    const query = normalize(state.query);
    if (!query) { state.results = filtered; return getState(); }
    state.results = filtered.map((video, index) => ({ video, index, score: getSearchScore(video, query) })).sort((a,b) => b.score - a.score || a.index - b.index).map(item => item.video);
    return getState();
  }
  ...
  function updateUrl() {
    ...
    if (state.category && normalize(state.category) !== "all") params.set("category", state.category);
  ...
  function readUrlState() {
    ...
    const validCategory = getCategories().find(category => normalize(category) === normalize(state.category));
    state.category = validCategory || "All";
  }

  function setQuery(query) { state.query = clean(query); updateUrl(); return apply(); }
  function setCategory(category) { state.category = clean(category) || "All"; updateUrl(); return apply(); }
  function clear() { state.query = ""; state.category = "All"; updateUrl(); return apply(); }

  function getState() {
    return { allVideos: [...state.allVideos], results: [...state.results], query: state.query, category: state.category };
  }
  ...
  function initialize() {
    ...
    readUrlState();
    apply();
    window.XKissSearch = { getState, getCategories, setQuery, setCategory, clear };
    document.dispatchEvent(new CustomEvent("xkiss:search-ready"));
  }
